...
export default function PartsOperationsLayout({ children }: { children: React.ReactNode }) {
  ...
  const menuItems = [
    ...
    { name: "RFQ & Comparison", path: "/inventory/rfq", icon: "📄" },
  ...
  return (
    <div className="flex min-h-screen bg-[#06080C] text-white font-sans selection:bg-[#1E4FD8] selection:text-white">
      ...
      <aside className="w-72 border-r border-slate-900 flex flex-col justify-between p-6 relative shrink-0"
        ...
        <div className="space-y-6 relative z-10">
          ...
          <div className="flex items-center gap-4 border-b border-slate-900 pb-5">
            <div className="w-12 h-12 rounded-xl bg-slate-950 border border-slate-800 flex items-center justify-center shadow-[0_0_15px_rgba(30,79,216,0.3)]">
              <span className="text-2xl font-black text-yellow-400">⚙️</span>
            </div>
            <div>
              <h2 className="text-lg font-black tracking-widest uppercase">PARTSMAN</h2>
              <p className="text-[10px] text-[#1E4FD8] font-mono font-bold tracking-widest uppercase">PARTS OPERATIONS</p>
            </div>
          </div>

          {/* Client Account Company Profile Badge */}
          <div className="p-3 bg-slate-950/80 border border-slate-800 rounded-xl space-y-1">
            <span className="text-[9px] font-mono text-[#F59E0B] uppercase tracking-widest font-black block">● LOGGED CLIENT ACCOUNT</span>
            <div className="font-bold text-xs text-white truncate">{clientCompany}</div>
          </div>

          {/* Navigation Links */}
          <nav className="space-y-2">
            <p className="text-[10px] text-slate-500 font-mono uppercase tracking-widest pl-4 mb-3">Operations Dashboard</p>
            {menuItems.map((item) => {
              const isActive = location === item.path || (item.path === "/inventory" && (location === "/inventory" || location === "/products"));
              return (
                <Link
                  key={item.path}
                  href={item.path}
                  className={`flex items-center gap-3 px-4 py-3 rounded-xl font-semibold transition-all duration-200 group relative overflow-hidden text-sm ${
                    isActive 
        ...
        <div className="space-y-4 relative z-10">
          <Link
            href="/portal"
            className="flex items-center justify-center gap-2 w-full p-3 border border-slate-800 hover:border-slate-700 text-slate-400 hover:text-white rounded-xl font-bold transition text-xs uppercase tracking-wider bg-white/2"
          >
            ← System Core Home
          </Link>
