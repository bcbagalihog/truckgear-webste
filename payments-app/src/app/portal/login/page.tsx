...
export default function PortalLogin() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    if (!cleanUser || !cleanPass) {
      setError('Please enter your Username / Account ID and Access Passcode');
      return;
    }
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(errData.message || 'ACCESS DENIED: Invalid credentials.');
      }

      const userData = await res.json();
      setLoadingStep(3);

      const isAdmin = userData?.role === 'admin' || cleanUser === 'admin' || cleanUser.includes('staff');
